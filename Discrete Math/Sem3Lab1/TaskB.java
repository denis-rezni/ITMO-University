...
//        System.out.println();
        ...
        for (int i = 0; i < n * (n - 1); i++) {
            ...
            if (flag) {
                ...
                int pointer = 2;
                while (pointer + 1 < arr.length && (!hasEdge[arr[0]][arr[pointer]]
                        || (!hasEdge[arr[1]][arr[pointer + 1]]))) {
                    pointer++;
                }
                if (pointer == arr.length - 1) {
                    int secondPointer = 2;
                    while (secondPointer < arr.length && !hasEdge[arr[0]][arr[secondPointer]]) {
                        secondPointer++;
                    }
                    swap(1, secondPointer, arr);
                } else {
                    swap(1, pointer, arr);
                }
